...
#if defined(NDEBUG)
#error "Madcoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umdc) are considered zero fee (for relaying and mining)
...
bool IsPOWActive (int nHeight){
	if (nHeight > Params().LAST_POW_BLOCK() && nHeight < Params().REACTIVATE_POW_BLOCK()) {
		return false;
	} else {
		return true;
	}
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight > 0 && nHeight <= 132) {
        nSubsidy = 1 * COIN;
    } else if (nHeight > 132 && nHeight <= 138) {
        nSubsidy = 10000 * COIN;
    } else if (nHeight > 38 && nHeight <= 2000) {
        nSubsidy = 1 * COIN;
    } else if (nHeight > 2000 && nHeight <= 20000) {
        nSubsidy = 25 * COIN;
    } else if (nHeight > 20000 && nHeight <= 30000) {
        nSubsidy = 15 * COIN;
    } else if (nHeight >= Params().REACTIVATE_POW_BLOCK()) {
        nSubsidy = 0;		
    } else {
        nSubsidy = 3 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    // No rewards for swap wallet.
    if (nHeight < Params().REACTIVATE_POW_BLOCK() || blockValue == 0)
        return 0;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (IsPOWActive(pindex->nHeight) && block.IsProofOfStake())
        return state.DoS(100, error("ConnectBlock() : PoS period not active"),
            REJECT_INVALID, "PoS-early");
	
    if (!IsPOWActive(pindex->nHeight) && block.IsProofOfWork())
...
{
	if (pindexPrev->nHeight < 3000) {
    if (pindexPrev == NULL)
        return error("%s : null pindexPrev for block %s", __func__, block.GetHash().ToString().c_str());

    unsigned int nBitsRequired = GetNextWorkRequired(pindexPrev, &block);

    if (block.IsProofOfWork() && (pindexPrev->nHeight + 1 <= Params().LAST_POW_BLOCK()) && (pindexPrev->nHeight + 1 >= Params().REACTIVATE_POW_BLOCK())) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MDC: We use certain sporks during IBD, so check to see if they are
