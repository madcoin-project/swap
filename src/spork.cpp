...
// MDC: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_4_PROPOSAL_VETO) r = SPORK_4_PROPOSAL_VETO_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_4_PROPOSAL_VETO") return SPORK_4_PROPOSAL_VETO;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_4_PROPOSAL_VETO) return "SPORK_4_PROPOSAL_VETO";
