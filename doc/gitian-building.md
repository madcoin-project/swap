...
*Setup instructions for a gitian build of Madcoin using a Debian VM or physical system.*
...
- [Building Madcoin](#building-madcoin)
...
Clone the git repositories for madcoin and gitian and then checkout the madcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the madcoin repository
...
To build Madcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the madcoin repository.
...
```bash
    ...
    From https://github.com/madcoin/madcoin
...
For example:
```bash
URL=https://github.com/konez2k/madcoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/madcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/madcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/madcoin-build.assert
```
...
[madcoin/gitian.sigs](https://github.com/madcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
